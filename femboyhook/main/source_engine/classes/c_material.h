...
public:
	void set_float_value( float value ) {
		...
		return ( *( fn** )this )[ this->e_indexes::_set_float_value ]( this, value );
	}

	void set_component_value(float value, int component) {
		using fn = void( __thiscall* )( void*, float, int );
		return ( *( fn** )this )[ this->e_indexes::_set_component ]( this, value, component );
	}

	void set_vector_value( float x, float y, float z ) {
		using fn = void( __thiscall* )(void*, float, float, float );
		return ( *( fn** )this )[ this->e_indexes::_set_vector_value ]( this, x, y, z );
	}

	 void set_vector_component( float* value )
	{
		set_component_value( value[ 0 ], 0 );
		set_component_value( value[ 1 ], 1 );
		set_component_value( value[ 2 ], 2 );
		set_component_value( value[ 3 ], 3 );
	}
};
...
public:
	...
	virtual preview_image_retval_t get_preview_image_properties( int* width, int* height, image_format_t* image_format,
	                                                             bool* is_translucent ) const                                         = 0;
	virtual preview_image_retval_t get_preview_image( unsigned char* data, int width, int height, image_format_t image_format ) const = 0;
	...
	virtual c_material_var* find_var( const char* var_name, bool* found, bool complain = true )                                       = 0;
	...
	virtual c_material_var* find_var_fast( char const* var_name, unsigned int* token )                                                = 0;
