...
constexpr crc32_t crc_table[ 256 ] = {
...
void crc32::process_buffer( crc32_t* crc, const void* buffer, int buffer_size )
{
	crc32_t new_crc     = *crc;
...
just_a_few:
	...
	switch ( buffer_size ) {
	case 7:
		new_crc = crc_table[ *pb++ ^ ( unsigned char )new_crc ] ^ ( new_crc >> 8 );
	...
	case 4:
		new_crc ^= little_long( *( crc32_t* )pb );
		new_crc   = crc_table[ ( unsigned char )new_crc ] ^ ( new_crc >> 8 );
		new_crc   = crc_table[ ( unsigned char )new_crc ] ^ ( new_crc >> 8 );
		new_crc   = crc_table[ ( unsigned char )new_crc ] ^ ( new_crc >> 8 );
		new_crc   = crc_table[ ( unsigned char )new_crc ] ^ ( new_crc >> 8 );
		*crc = new_crc;
	...
	//
	// The low-order two bits of pb and buffer_size in total control the
	// upfront work.
	//
	...
	while ( main-- ) {
		...
		new_crc = crc_table[ ( unsigned char )new_crc ] ^ ( new_crc >> 8 );
		new_crc = crc_table[ ( unsigned char )new_crc ] ^ ( new_crc >> 8 );
		new_crc ^= little_long( *( crc32_t* )( pb + 4 ) );
		new_crc = crc_table[ ( unsigned char )new_crc ] ^ ( new_crc >> 8 );
		new_crc = crc_table[ ( unsigned char )new_crc ] ^ ( new_crc >> 8 );
		new_crc = crc_table[ ( unsigned char )new_crc ] ^ ( new_crc >> 8 );
		new_crc = crc_table[ ( unsigned char )new_crc ] ^ ( new_crc >> 8 );
